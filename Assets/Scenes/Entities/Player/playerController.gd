...
@export var interactableReach = 4.0
...
func _process(delta: float) -> void:
	camera.basis = camera.basis.slerp(Basis.IDENTITY.rotated(Vector3.UP, PI).rotated(Vector3.RIGHT, camYRot), min(15.0*delta, 1.0))
	var camGlobal = global_position+camPos
	camGlobal.y = camera.global_position.y
	...
	camera.position.z = min(sin(camYRot)*neckLength, wallDistance)+camPos.z
	camera.position.y = cos(camYRot)*neckLength+camPos.y-neckLength
